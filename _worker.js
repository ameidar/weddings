...
async function rsvpSubmitApi(request, env) {
  ...
  const message = status === 'אישר' ? `תודה! אישרנו הגעה עבור ${guest['כמות מוזמנים']} משתתפים.` : status === 'אולי' ? 'תודה על העדכון. סימנו “לא ידוע כרגע” ונזכיר בהמשך.' : 'תודה על העדכון. סימנו שלא תוכלו להגיע.';
