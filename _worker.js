...
async function rsvpPage(request, env) {
  ...
  return htmlResponse(`<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>אישור הגעה - ${eventName}</title><style>@import url("https://fonts.googleapis.com/css2?family=Assistant:wght@300;400;500;600;700;800&display=swap");*{box-sizing:border-box}body{font-family:Assistant,Arial,sans-serif;background:linear-gradient(180deg,#f5efe7,#fbfaf7);margin:0;color:#21160f}.wrap{max-width:430px;margin:0 auto;min-height:100vh;background:#fff;box-shadow:0 18px 60px #2b170d20}.hero{min-height:220px;background:${bg ? `linear-gradient(180deg,#0002,#0007),url(${bg}) center/cover` : 'linear-gradient(135deg,#1e130c,#8b6f47,#d8b07a)'};color:#fff;display:flex;align-items:end;justify-content:center;text-align:center;padding:28px 24px}.hero h1{margin:0;font-size:38px;letter-spacing:.5px;line-height:1}.hero p{margin:8px 0 0;font-size:15px;opacity:.92}.card{padding:22px 22px 30px}.hello{text-align:center;font-size:18px;margin:0 0 18px}.details{display:grid;gap:12px;margin:16px 0}.detail{display:grid;grid-template-columns:34px 1fr;gap:10px;align-items:center;border-bottom:1px solid #eee2d5;padding:0 0 12px}.ico{width:34px;height:34px;border-radius:50%;background:#f4eadc;color:#8b5e2b;display:flex;align-items:center;justify-content:center;font-size:19px}.detail b{display:block;font-size:16px}.detail span{display:block;color:#6f6259;font-size:14px;margin-top:2px}.rings{text-align:center;color:#b78a50;font-size:28px;margin:6px 0 2px}.choice-title{text-align:center;font-weight:900;margin:18px 0 10px}.choices{display:grid;grid-template-columns:1fr 1fr 1.35fr;gap:8px;direction:rtl}.choice{border:1px solid #e6d8c7;background:#fff;border-radius:16px;min-height:54px;font-size:16px;font-weight:900;color:#3a2a1f;cursor:pointer}.choice.active,.choice:hover{background:#1f7a4d;color:#fff;border-color:#1f7a4d}.choice.maybe.active,.choice.maybe:hover{background:#c98222;border-color:#c98222}.choice.no.active,.choice.no:hover{background:#a43c34;border-color:#a43c34}.maybeConfirm{display:none;margin-top:14px;border:1px solid #f3c47c;background:#fff8ed;border-radius:16px;padding:14px;text-align:center}.maybeConfirm.open{display:block}.maybeConfirm b{display:block;font-size:17px;margin-bottom:7px}.maybeConfirm p{margin:0 0 12px;color:#6f4a1f;font-weight:700}.maybeActions{display:grid;grid-template-columns:1.25fr 1fr;gap:8px}.qtyBox,.submitBox{display:none;margin-top:14px}.qtyBox.open,.submitBox.open{display:block}.qtyBox label,.submitBox label{font-weight:800}.qtyRow{display:grid;grid-template-columns:1fr 1.25fr;gap:10px;margin-top:8px}input{width:100%;height:52px;border:1px solid #d8c8b5;border-radius:14px;font-size:20px;padding:0 12px;text-align:center}.cta{width:100%;height:54px;border:0;border-radius:16px;background:#1f7a4d;color:#fff;font-size:18px;font-weight:900;cursor:pointer}.cta.secondary{background:#efe6da;color:#2d2118}.actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:16px}.actions a{text-decoration:none;text-align:center;border:1px solid #e6d8c7;border-radius:15px;padding:13px 8px;color:#3a2a1f;font-weight:900;background:#fff}.status{font-weight:900;margin-top:14px;white-space:pre-wrap;text-align:center;color:#1f7a4d}.brand{text-align:center;color:#9b8c7d;font-size:12px;padding:12px 0 20px}.hidden{display:none}@media(max-width:460px){.wrap{max-width:none}.hero h1{font-size:34px}.choices{grid-template-columns:1fr}.actions{grid-template-columns:1fr}}</style></head><body><main class="wrap"><section class="hero"><div><h1>${eventName}</h1><p>${eventType} · ${eventDate}</p></div></section><section class="card"><p class="hello">שלום ${name}, נשמח לעדכון הגעה לאירוע. אם עדיין לא בטוחים — אפשר לבחור “לא ידוע כרגע”; במקרה כזה נבקש אישור שכרגע לא ניקח אתכם בחשבון, ואם יהיה שינוי תוכלו לעדכן כאן.</p><div class="details"><div class="detail"><div class="ico">📅</div><div><b>${eventDate}</b><span>תאריך האירוע</span></div></div><div class="detail"><div class="ico">📍</div><div><b>${venue}</b><span>${address || 'כתובת תעודכן בהמשך'}</span></div></div></div><div class="rings">💍</div><div class="choice-title">כמה מגיעים?</div><div class="choices"><button id="yesBtn" class="choice" onclick="selectStatus('אישר')">נגיע</button><button id="maybeBtn" class="choice maybe" onclick="selectStatus('אולי')">לא ידוע כרגע</button><button id="noBtn" class="choice no" onclick="selectStatus('לא מגיע')">לא נוכל להגיע</button></div><section id="qtyBox" class="qtyBox"><label>בחירת כמות משתתפים</label><div class="qtyRow"><input id="qty" type="number" min="1" max="99" value="${qty}"><button class="cta" onclick="submitRsvp()">שליחת אישור</button></div></section><section id="maybeConfirm" class="maybeConfirm"><b>האם זה בסדר מבחינתכם שלא ניקח אתכם בחשבון כרגע?</b><p>אם יהיה שינוי, אפשר לחזור לקישור או לעדכן בהודעה.</p><div class="maybeActions"><button class="cta secondary" onclick="submitRsvp(true)">כן, כרגע אל תיקחו בחשבון</button><button class="cta" onclick="selectStatus('אישר')">בעצם נגיע</button></div></section><section id="submitBox" class="submitBox"><label>לאחר הבחירה, לחצו לשליחת התשובה</label><button class="cta secondary" onclick="submitRsvp(false)">שליחת תשובה</button></section><div class="actions"><a href="${navLink}" target="_blank" rel="noopener">ניווט לאירוע</a><a href="${calLink}" target="_blank" rel="noopener">הוספה ליומן</a></div><div id="status" class="status">סטטוס נוכחי: ${currentStatus}</div></section><div class="brand">powered by Orma</div></main><script>const token=${JSON.stringify(token)};let selected='';function setActive(){document.getElementById('yesBtn').classList.toggle('active',selected==='אישר');document.getElementById('maybeBtn').classList.toggle('active',selected==='אולי');document.getElementById('noBtn').classList.toggle('active',selected==='לא מגיע');document.getElementById('qtyBox').classList.toggle('open',selected==='אישר');document.getElementById('maybeConfirm').classList.toggle('open',selected==='אולי');document.getElementById('submitBox').classList.toggle('open',selected==='לא מגיע')}function selectStatus(s){selected=s;setActive();document.getElementById('status').textContent=s==='אישר'?'בחרו כמות ולחצו שליחת אישור':s==='אולי'?'נא לאשר שכרגע לא ניקח אתכם בחשבון, ואם יהיה שינוי תוכלו לעדכן':'לחצו שליחת תשובה כדי לעדכן את המערכת'}async function submitRsvp(maybeExcluded=false){const el=document.getElementById('status');if(!selected)selected='אישר';if(selected==='אולי'&&!maybeExcluded){el.textContent='כדי לסמן לא ידוע כרגע, אשרו שכרגע לא ניקח אתכם בחשבון.';return;}el.textContent='שומר תשובה...';try{const body={token,status:selected,count:selected==='אישר'?document.getElementById('qty').value:0,maybeExcluded:!!maybeExcluded};const r=await fetch('/api/rsvp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'שמירה נכשלה');el.textContent=d.message||'התשובה נשמרה, תודה רבה!'}catch(e){el.textContent='לא הצלחנו לשמור: '+e.message}}setActive();</script></body></html>`);
}
...
async function rsvpSubmitApi(request, env) {
  ...
  guest['הערות'] = `${guest['הערות'] ? guest['הערות'] + ' | ' : ''}עודכן דרך קישור RSVP בתאריך ${new Date().toLocaleString('he-IL')}${status === 'אולי' && body.maybeExcluded ? ' · אישר/ה שכרגע לא לקחת בחשבון' : ''}`;
  state.updatedAt = new Date().toISOString();
  await env.EVENTS_KV.put(key, JSON.stringify(state));
  const message = status === 'אישר' ? `תודה! אישרנו הגעה עבור ${guest['כמות מוזמנים']} משתתפים.` : status === 'אולי' ? 'תודה על העדכון. סימנו “לא ידוע כרגע” וכרגע לא ניקח אתכם בחשבון. אם יהיה שינוי, אפשר לחזור לקישור או לעדכן אותנו בהודעה.' : 'תודה על העדכון. סימנו שלא תוכלו להגיע.';
...
function guestFeePercent(env) {
  const raw = env.ORMA_GUEST_FEE_PERCENT ?? env.PAYMENT_GUEST_FEE_PERCENT ?? 2;
  const pct = Number(raw);
  return Number.isFinite(pct) && pct >= 0 ? Math.min(pct, 20) : 2;
}
function calculateGuestFee(amount, env) {
  const giftAmount = Math.max(0, Math.round((Number(amount) || 0) * 100) / 100);
  const feePercent = guestFeePercent(env);
  const feeAmount = Math.round(giftAmount * feePercent) / 100;
  const totalAmount = Math.round((giftAmount + feeAmount) * 100) / 100;
  return { giftAmount, feePercent, feeAmount, totalAmount };
}
...
async function paymentChoicePage(request, env) {
  ...
  const feePercent = guestFeePercent(env);
  return htmlResponse(`<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>בחירת סכום מתנה</title><style>body{font-family:Assistant,Arial,sans-serif;background:#171411;color:#f7efe6;margin:0}.box{max-width:540px;margin:28px auto;background:#211914;border:1px solid #d8b07a55;border-radius:24px;padding:24px;box-shadow:0 24px 80px #0008}h1{margin-top:0;color:#d8b07a}.chips,.installments{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:14px 0}.chips button,.installments button,.main{height:52px;border:0;border-radius:14px;background:#d8b07a;color:#1a0f08;font-weight:900;font-size:18px;cursor:pointer}.chips button,.installments button{background:#30251d;color:#f7efe6;border:1px solid #d8b07a55}.chips button.active,.installments button.active{background:#d8b07a;color:#1a0f08}label{display:block;margin:16px 0 8px;font-weight:900;color:#ead7bd}input,select{width:100%;box-sizing:border-box;height:56px;border-radius:14px;border:1px solid #d8b07a55;background:#110f0d;color:#fff;font-size:22px;padding:0 14px}select{font-size:18px}.summary{margin:18px 0;background:#110f0d;border:1px solid #d8b07a55;border-radius:18px;padding:14px}.trust{margin-top:18px;background:#18120f;border:1px solid #ffffff18;border-radius:18px;padding:14px;color:#ead7bd;font-size:14px;line-height:1.55}.trust b{color:#fff}.trust a{color:#d8b07a;font-weight:900;text-decoration:none}.trust .mini{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}.trust .mini span{border:1px solid #d8b07a55;border-radius:999px;padding:6px 10px;background:#110f0d}.row{display:flex;justify-content:space-between;gap:12px;margin:8px 0;color:#ead7bd}.row.total{border-top:1px solid #d8b07a55;padding-top:12px;margin-top:12px;color:#fff;font-size:20px;font-weight:900}.note{color:#c9b8a5;font-size:14px;line-height:1.45}.status{white-space:pre-wrap;margin-top:12px;color:#ffc7c2}@media(max-width:520px){.chips,.installments{grid-template-columns:repeat(2,1fr)}.box{margin:14px;padding:20px}}</style></head><body><main class="box"><h1>בחירת סכום מתנה</h1><p>שלום ${payerName}, תודה שאתם חלק מ${eventName} ❤️</p><p class="note">בחרו סכום חופשי שמתאים לכם. לפני המעבר לסליקה נציג בצורה שקופה את המתנה, עמלת השירות והסכום הכולל לחיוב.</p><label for="amount">סכום מתנה בש״ח</label><input id="amount" type="number" min="1" step="1" placeholder="הקלידו סכום לבחירתכם"><div class="chips">${[100,180,250,360,500,1000].map(n=>`<button type="button" onclick="setAmount(${n},this)">₪${n}</button>`).join('')}</div><label>פריסה לתשלומים</label><div class="installments">${[1,2,3,6,10,12].map(n=>`<button type="button" data-payments="${n}" onclick="setPayments(${n},this)">${n===1?'תשלום אחד':n+' תשלומים'}</button>`).join('')}</div><select id="payments"><option value="1">תשלום אחד</option><option value="2">2 תשלומים</option><option value="3">3 תשלומים</option><option value="6">6 תשלומים</option><option value="10">10 תשלומים</option><option value="12">12 תשלומים</option></select><div class="summary"><div class="row"><span>מתנה</span><b id="giftLine">₪0</b></div><div class="row"><span>עמלת שירות (${feePercent}%)</span><b id="feeLine">₪0</b></div><div class="row total"><span>סה״כ לחיוב</span><b id="totalLine">₪0</b></div></div><p class="note">עמלת השירות מוצגת מראש ומתווספת לסכום החיוב. כספי המתנה ממשיכים להיסלק דרך ספק התשלום של בעל האירוע; Orma לא שומרת פרטי אשראי.</p><section class="trust"><b>התשלום מאובטח ומתבצע באמצעות ספק התשלום Morning.</b><br>פרטי האשראי אינם נשמרים ב־Orma. לאחר התשלום תועברו לאישור תשלום, והמתנה תעודכן בתיק האירוע.<div class="mini"><span>תשלום בכרטיס אשראי</span><span>דף סליקה מאובטח</span><span>שקיפות מלאה בעמלה</span></div><p style="margin:10px 0 0">צריכים עזרה? <a href="https://wa.me/972533009742" target="_blank" rel="noopener">פנו אלינו בוואטסאפ</a></p></section><button class="main" onclick="go()">המשך לתשלום מאובטח</button><div id="status" class="status"></div></main><script>const token=${JSON.stringify(token)};const feePercent=${JSON.stringify(feePercent)};const money=n=>'₪'+(Math.round((Number(n)||0)*100)/100).toLocaleString('he-IL');function calc(){const gift=Number(document.getElementById('amount').value)||0;const fee=Math.round(gift*feePercent)/100;const total=Math.round((gift+fee)*100)/100;giftLine.textContent=money(gift);feeLine.textContent=money(fee);totalLine.textContent=money(total);return{gift,fee,total}}function setAmount(n,btn){amount.value=n;document.querySelectorAll('.chips button').forEach(b=>b.classList.remove('active'));btn&&btn.classList.add('active');calc()}function setPayments(n,btn){payments.value=n;document.querySelectorAll('.installments button').forEach(b=>b.classList.remove('active'));btn&&btn.classList.add('active')}document.getElementById('amount').addEventListener('input',calc);calc();setPayments(1,document.querySelector('[data-payments="1"]'));async function go(){const s=document.getElementById('status');const c=calc();const maxPayments=Number(document.getElementById('payments').value)||1;if(c.gift<=0){s.textContent='נא להזין סכום מתנה גדול מאפס';return} s.textContent='מכין קישור תשלום מאובטח...';try{const r=await fetch('/api/morning/create-gift-link',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({token,giftAmount:c.gift,maxPayments})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'יצירת קישור נכשלה');location.href=d.url}catch(e){s.textContent='לא הצלחנו לפתוח תשלום: '+e.message}}</script></body></html>`);
}
...
async function createMorningGiftLink(request, env) {
  try {
    ...
    const fee = calculateGuestFee(body.giftAmount ?? body.amount, env);
    if (fee.giftAmount <= 0) return jsonResponse({ ok: false, error: 'נא להזין סכום מתנה גדול מאפס' }, 400);
    const result = await createMorningPaymentForm(env, request, { ...payment, eventId: claims.eventId, paymentId: claims.paymentId, amount: fee.totalAmount, maxPayments: Number(body.maxPayments) || 1, description: payment.description || 'מתנה לאירוע' });
    payment.chosenGiftAmount = fee.giftAmount; payment.guestFeePercent = fee.feePercent; payment.guestFeeAmount = fee.feeAmount; payment.chosenAmount = fee.totalAmount; payment.amount = fee.giftAmount; payment.chosenPayments = Number(body.maxPayments) || 1; payment.lastGeneratedPaymentUrl = result.url; payment.providerStatus = 'guest_amount_selected'; payment.updatedAt = new Date().toISOString();
    ...
    return jsonResponse({ ...result, giftAmount: fee.giftAmount, feeAmount: fee.feeAmount, totalAmount: fee.totalAmount, feePercent: fee.feePercent });
    ...
    if (eventId && paymentId) {
      ...
      if (state && Array.isArray(state.payments) && paidSignal) {
        ...
        if (p && p.status !== 'paid') {
          ...
          const chargedAmount = Number(body.amount || body.sum || body.total || p.chosenAmount || p.amount) || Number(p.chosenAmount || p.amount) || 0;
          const paidAmount = Number(p.chosenGiftAmount || p.amount) || chargedAmount;
          if (paidAmount) p.amount = paidAmount;
          if (chargedAmount) p.chargedAmount = chargedAmount;
          state.walletTx = Array.isArray(state.walletTx) ? state.walletTx : [];
          if (!state.walletTx.some(t => t.paymentId === paymentId)) state.walletTx.unshift({ date: p.paidAt, amount: paidAmount, type: 'income', note: `תשלום Morning התקבל: ${p.payerName || ''} · ${p.description || ''}${p.guestFeeAmount ? ' · עמלת שירות שנגבתה מהאורח: ₪'+p.guestFeeAmount : ''}`, paymentId });
